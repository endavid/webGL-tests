...
SphericalHarmonics.prototype.setupSphericalSamples = function()
{
  ...
  for (a = 0; a < this.numSamplesSqrt; ++a) {
    for (b = 0; b < this.numSamplesSqrt; ++b) {
      ...
      // This shape will avoid very packed sampling around the poles
      theta = 2.0 * math.acos(math.sqrt(1.0 - x));
      // If we wanted to sample the hemisphere, you need to scale X so you
      // don't take less samples around the equator!
      // https://www.dropbox.com/s/ganxs7t9x7os8ay/Screenshot%202017-03-10%2020.44.25.png?dl=0
      // theta = 2.0 * math.acos(math.sqrt(2.0 - x)/math.sqrt(2.0));
      phi = 2.0 * math.pi * y;
      var s = {};
      s.sph = [theta, phi];
...
SphericalHarmonics.prototype.projectPolarFn = function(fn)
{
  ...
  var factor = weight / this.numSamples;
